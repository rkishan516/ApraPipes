...
__global__ void applaunchMul(const Npp32f* src, int nSrcStep, Npp8u* dst_rgb, int rDstStep, int width, int height)
{
	...
	int dst_offset = y_ * rDstStep +  (3*x);
	int offset = y_* nSrcStep + (x);
    int amp = src[offset] * 255.0;
    dst_rgb[dst_offset+0] = static_cast<Npp8u>(jet[amp][0]);
    dst_rgb[dst_offset+1] = static_cast<Npp8u>(jet[amp][1]);
    dst_rgb[dst_offset+2] = static_cast<Npp8u>(jet[amp][2]);
}

__global__ void appRGBAToRGBSaver(const Npp8u* src, int nSrcStep, Npp8u* dst_rgb, int rDstStep, int width, int height)
{
	int x = blockIdx.x * blockDim.x + threadIdx.x;
	int y_ = blockIdx.y * blockDim.y + threadIdx.y;
...
void lanuchAPPRGBAToRGBSaver(const Npp8u* src,int nSrcStep, Npp8u* dst,int rDstStep, NppiSize oSizeROI, cudaStream_t stream){
	dim3 block(32, 32); 
	dim3 grid((oSizeROI.width + block.x - 1) / (1 * block.x), (oSizeROI.height + block.y - 1) / block.y);
	appRGBAToRGBSaver <<<grid, block, 0, stream>>> (src, nSrcStep, dst, rDstStep, oSizeROI.width, oSizeROI.height);
}